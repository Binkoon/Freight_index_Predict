...
def SCFI_Crawling():
    ...
    try:
        ...
        yesterday_day = str(yesterday.day).zfill(2)  # 01, 02 형태로 만들어야함. 캘린더 표기법때문에
        ...
        while True:
            try:
                ...
                # 클래스명에 'isTotal'이 포함되어 있으면 루핑 종료
                tr_class = tr_element.get_attribute("class")
                if 'isTotal' in tr_class:
                    print(f"tr[{tr_index}]에서 isTotal 클래스 발견, 데이터 수집 종료")
                    break
                ...
                print(f"tr[{tr_index}] 처리 중 오류 또는 더 이상 데이터 없음: {e}")
                break
